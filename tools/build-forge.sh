#!/usr/bin/env bash
# Build the Forge (MC 1.20.1) artifact for cfx-compat-create.
# Mirrors cfx-compat-exposure/tools/build-forge.sh.
#
# Usage:
...
NEW_JAR="${PROJECT_DIR}/forge/build/libs/cfx-compat-create-${label}-mc${mc_ver}-forge.jar"
if [[ ! -f "${NEW_JAR}" ]]; then
...
if [[ -z "${MC_INSTANCE}" ]]; then
    echo "MC_1_20_1_FORGE_PATH is not set; skipping instance deploy"
    ...
    if [[ -d "${MODS_DIR}" ]]; then
        rm -f "${MODS_DIR}"/cfx-compat-create-*-mc*-forge.jar
        echo "  removed old cfx-compat-create Forge jars"
    fi
